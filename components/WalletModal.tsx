    ...
    return (
        <div className="text-slate-300">
            <h2 className="text-sm font-semibold mb-6">
                <span className="text-slate-400">Crypto</span> &gt; {cryptoName}
            </h2>
            <div className="space-y-6">
                <div>
                    <label htmlFor="chain" className="block text-sm font-semibold text-slate-400 mb-2">Chain</label>
                    <div className="relative">
                        <select
                            id="chain"
                            className="w-full bg-[#1e293b] border border-slate-700 rounded-lg px-4 py-3 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    <label htmlFor="wallet-address" className="block text-sm font-semibold text-slate-400 mb-2">Wallet Address</label>
                    <div className="relative">
                        <input
                            type="text"
                            id="wallet-address"
                            ...
                            className="w-full bg-[#1e293b] border border-slate-700 rounded-lg pl-4 pr-12 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                ...
                <div>
                    <label htmlFor="amount" className="block text-sm font-semibold text-slate-400 mb-2">Amount</label>
                    <div className="relative flex items-center">
                        <input
                            ...
                            placeholder="0"
                            className="w-full bg-[#1e293b] border border-slate-700 rounded-lg pl-4 pr-32 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        />
                        <button 
            ...
            <div className="mt-8 flex items-center gap-3">
                 <button onClick={onBack} className="p-3 bg-[#1e293b] hover:bg-slate-700 rounded-lg">
...
const WalletModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={closeModal}>
            <div className="bg-[#141c2f] rounded-lg shadow-xl w-full max-w-md text-slate-300" onClick={e => e.stopPropagation()}>
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex border-b border-slate-700">
                           <button className={`px-4 py-2 font-semibold text-white border-b-2 border-blue-500`}>
                                Withdraw
                            </button>
                        </div>
                        <button onClick={closeModal} className="text-3xl font-light text-slate-400 hover:text-white">&times;</button>
                    </div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {['Bitcoin (BTC)', 'Ethereum (ETH)', 'Litecoin (LTC)', 'Solana (SOL)', 'Tether (USDT)', 'USD Coin (USDC)', 'Tron (TRX)', 'Ripple (XRP)'].map(crypto => (
                                            <button 
                                              ...
                                              className="bg-[#1e293b] hover:bg-slate-700 p-4 rounded-lg text-center flex flex-col items-center justify-center gap-2"
                                            >
