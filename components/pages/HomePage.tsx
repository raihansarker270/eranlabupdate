...
const FaqAccordionItem: React.FC<{ item: FaqItem }> = ({ item }) => {
    ...
    return (
        <div className="border-b border-slate-200 dark:border-slate-700">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full text-left flex justify-between items-center p-6 hover:bg-slate-100/50 dark:hover:bg-slate-700/20">
                <span className="font-semibold text-lg text-slate-900 dark:text-white">{item.question}</span>
                <span className={`transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
                    <i className="fas fa-chevron-down"></i>
                </span>
            </button>
            {isOpen && <div className="px-6 pb-6 pt-0 text-slate-600 dark:text-slate-400">{item.answer}</div>}
        </div>
    );
};
...
const HomePageContent: React.FC<HomePageProps> = ({ onLogin }) => {
  return (
    <div className="bg-white dark:bg-[#0b111e] text-slate-700 dark:text-slate-300">
        ...
        <section className="py-20 bg-slate-50 dark:bg-[#141c2f] text-center">
            ...
            <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 px-8">
                 {[{img: 'https://i.imgur.com/T0bC2zZ.jpeg', text: 'Easily Sign up'}, {img: 'https://i.imgur.com/4l3z4P4.jpeg', text: 'Complete Tasks'}, {img: 'https://i.imgur.com/uJgJa8Z.jpeg', text: 'Get Paid'}].map((item, i) => (
                    <div key={i} className="bg-white dark:bg-[#1e293b] rounded-lg overflow-hidden shadow-lg transform hover:-translate-y-2 transition-transform duration-300 relative">
        ...
        {/* Rewards Section */}
        <section className="py-20 bg-white dark:bg-[#0f172a] text-center">
        ...
        <section className="py-20 bg-slate-50 dark:bg-[#141c2f] text-center">
             ...
             <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 px-8">
                <div className="text-center">
                    <div className="text-5xl font-bold text-blue-500 dark:text-blue-400">415,942</div>
                    <div className="text-slate-600 dark:text-slate-400 mt-2">Total Users</div>
                </div>
                 <div className="text-center">
                    <div className="text-5xl font-bold text-blue-500 dark:text-blue-400">1,438,691</div>
                    <div className="text-slate-600 dark:text-slate-400 mt-2">Tasks Completed</div>
                </div>
                 <div className="text-center">
                    <div className="text-5xl font-bold text-blue-500 dark:text-blue-400">$3,741,246</div>
